...
export default function CloudAnimation() {
  useEffect(() => {
    ...
    async function loadModules() {
      // โหลดโมดูลจาก esm.sh (dynamic import)
      THREE = await import('https://esm.sh/three');
      OrbitControls = (await import('https://esm.sh/three/examples/jsm/controls/OrbitControls.js')).OrbitControls;
        ...
        renderer.domElement.addEventListener('click', (event) => {
          ...
          if (intersects.length > 0) {
            ...
            if (clickedObj.parent === cloudGroup) {
              const newGlobalRainState = !(cloud1.userData.isRaining && cloud2.userData.isRaining);
              cloud1.userData.isRaining = newGlobalRainState;
              cloud2.userData.isRaining = newGlobalRainState;
              if (cloud1.userData.rainGroup) cloud1.userData.rainGroup.visible = newGlobalRainState;
              if (cloud2.userData.rainGroup) cloud2.userData.rainGroup.visible = newGlobalRainState;

              const originalScale = clickedObj.scale.clone();
              clickedObj.scale.multiplyScalar(1.15);
              setTimeout(() => {
                clickedObj.scale.copy(originalScale);
              }, 150);
            }
          }
        });
  ...
  return (
    <>
      <div id="cloud-container" style={{ width: '100%', height: '400px', position: 'relative' }}></div>
      <div id="dateTime" className="text-white absolute top-2 right-2 z-10"></div>
      <div
        id="cloud-tooltip"
        className="opacity-0 transition-opacity duration-700 absolute bottom-2 left-1/2 transform -translate-x-1/2 text-white"
        style={{ pointerEvents: 'none' }}
      >
        Click cloud to toggle rain
      </div>
    </>
  );
}
